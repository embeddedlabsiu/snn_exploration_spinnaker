...
def create_test_set(img_size=48, num_classes=43):
...
def create_test_set_phone(img_size=48, num_classes=43):
    print("Current directory test ",os.getcwd())

    # Load test dataset
    test = pd.read_csv('GTSRB/GT-final_test_phone.csv', sep=';')
    X_test = []
    y_test = []
    i = 0
    #limit = 1000
    limit = 18
    for file_name, class_id in zip(list(test['Filename']), list(test['ClassId'])):
        img_path = os.path.join('GTSRB/Final_Test/From-phone/', file_name)
        img = grayscale_img(io.imread(img_path), img_size)
        X_test.append(img)
        y_test.append(class_id)
        i = i+1
...
def LISA_create_test_set_phone(img_size=48, num_classes=28):
    print("Current directory test ",os.getcwd())

    # Load test dataset
    test = pd.read_csv('LISA_dataset/LISA-final_test_phone.csv', sep=',')
    X_test = []
    y_test = []
    i = 0
    #limit = 1000
    limit = 8
    for file_name, class_id in zip(list(test['Filename']), list(test['ClassId'])):
        img_path = os.path.join('LISA_dataset/Final_Test/From-phone/', file_name)
        # img = grayscale_img(io.imread(img_path), img_size)
        img = preprocess_img(io.imread(img_path), img_size)
        X_test.append(img)
        y_test.append(class_id)
...
def GTSRB_create_test_set_segment(img_size=48, num_classes=43):
    print("Current directory test ",os.getcwd())
