...
do
	...
	do
		...
		do
			...
			sed -i "93s/.*/        self.sim.set_number_of_neurons_per_core(self.sim.SpikeSourcePoisson, $neurons_per_core) # limit number of neurons per core for input spike source /" /home/snn_toolbox/snntoolbox/simulation/target_simulators/spinnaker_target_sim.py
			sed -i "108s/.*/        self.sim.set_number_of_neurons_per_core(self.sim.SpikeSourcePoisson, $neurons_per_core) # limit number of neurons per core for input spike source/" /home/snn_toolbox/snntoolbox/simulation/target_simulators/spinnaker_target_sim.py
