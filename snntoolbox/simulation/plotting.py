    ...
    for i in range(num_layers):
        ...
        if 'correlation' in plot_keys:
            plot_layer_correlation(plot_vars['spikerates_n_l'][i][0].flatten(),
                                   ...
                                   # str('ANN-SNN correlations\n of layer '+name), #IRAKLIS
                                   str('CNN-SNN correlations \n of convolution layer'),
                                   config, newpath)
...
def plot_layer_correlation(rates, activations, title, config, path=None):
    ...
    #IRAKLIS
    fontsize = 24
    z = np.polyfit(activations, rates, 1)
    line = np.poly1d(z)
    plt.plot(activations,line(activations),"r--")
    print ("y=%.6fx+(%.6f)"%(z[0],z[1]))
    slope, intercept, r_value, p_value, std_err = stats.linregress(activations, rates)
    plt.annotate("Ratio = {:.2}, R² = {:.2}".format(z[0], r_value**2), xy=(1, 1),
                 xycoords='axes fraction', xytext=(-340, -25),
    ...
    plt.xlim([0, lim])
    plt.ylim([0, lim])
    plt.xlabel('CNN activations', fontsize=fontsize+2)
    plt.grid()
    # plt.ylabel('SNN spikerates [Hz]', fontsize=16)  #IRAKLIS
    plt.ylabel('SNN spikerates', fontsize=fontsize+2) 
    if path is not None:
...
def plot_pearson_coefficients(spikerates_batch, activations_batch, config,
    ...
    for i in range(len(labels)):
        if corr[i] > 0:
            plt.text(x = i + 0.1 , y = 0.9, s = str("{0:.2f}".format(corr[i])), size = 16)
        else:
            plt.text(x = i + 0.1 , y = 0.9, s = str("-{0:.2f}".format(abs(corr[i]))), size = 16)
...
def convert_Y_test(Y_test):
    ...
    for i in range(Y_test.shape[0]):
        decoded_datum = np.argmax(Y_test[i])
        Y_test_cl.append(decoded_datum)

    Y_test_cl = np.array(Y_test_cl)
    return Y_test_cl

counter_images = 0
# image_index = [0, 3, 15, 17, 18, 21, 22, 23]
image_index = 0
...
def plot_confusion_matrix(y_test, y_pred, path=None, class_labels=None):
    ...
    # if image_index<8:
    #     vocab_names = ['ADDED LANE', 'CURVE RIGHT', 'DIP', 'DO NOT PASS', 'INTERSECTION', 'KEEP RIGHT', 'LANE ENDS', 'MERGE', 'PEDESTRIAN CROSSING', 'RIGHT LANE MUST TURN', 'SIGNAL AHEAD', 'SLOW', 'SPEED LIMIT 15', 'SPEED LIMIT 25', 'SPEED LIMIT 30', 'SPEED LIMIT 35', 'SPEED LIMIT 40', 'SPEED LIMIT 45', 'SPEED LIMIT 50', 'SPEED LIMIT 55', 'SPEED LIMIT 65', 'STOP AHEAD', 'THRU MERGE LEFT', 'THRU MERGE RIGHT', 'TURN LEFT', 'TURN RIGHT', 'YIELD AHEAD', 'STOP']
    #     test_phone = pd.read_csv('/home/ces10/Downloads/keras/grayscale-GTSRB/R3-R4-demo-telco/LISA_dataset/LISA-final_test_phone.csv', sep=',')
    #     X_test_phone = []
    #     y_test_phone = []
    #     original_images = []
    #     for file_name, class_id in zip(list(test_phone['Filename']), list(test_phone['ClassId'])):
    #         img_path = os.path.join('/home/ces10/Downloads/keras/grayscale-GTSRB/R3-R4-demo-telco/LISA_dataset/Final_Test/From-phone/', file_name)
    #         original_images.append(io.imread(img_path))
    #         y_test_phone.append(class_id)

    #     y_test_phone = keras.utils.to_categorical(y_test_phone, 28)

    #     Y_test_cl = convert_Y_test(y_test_phone)
    ...
    #         plt.imshow(original_images[i-1+image_index])
    #         plt.axis('off')
    #         if (y_pred[i-1+image_index] == Y_test_cl[i-1+image_index]):
    #             plt.title('Correct prediction\n Predicted: '+str(vocab_names[y_pred[i-1+image_index]])+'\n Real: '+str(vocab_names[Y_test_cl[i-1+image_index]]), color='g', fontsize=11)
    #         else:
    #             plt.title('Wrong prediction\n Predicted: '+str(vocab_names[y_pred[i-1+image_index]])+'\n Real: '+str(vocab_names[Y_test_cl[i-1+image_index]]), color='r', fontsize=11)
    #         # plt.title('Correct', color='g', fontsize=11)
    #         # plt.title('Predicted: '+str(vocab_names[y_pred[i-1+image_index]])+'\n Real: '+str(vocab_names[Y_test_cl[i-1+image_index]]), color='r', fontsize=9)
